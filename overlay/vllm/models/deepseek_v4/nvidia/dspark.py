...
from .ops.fp8_einsum import deepseek_v4_fp8_einsum, deepseek_v4_fp8_einsum_config
from .ops.o_proj import compute_fp8_einsum_recipe  # noqa: F401
...
class DeepSeekV4DSparkAttention(nn.Module):
    ...
    def __init__(
        ...
        *,
        ...
        # Use the vendored deepseek_v4 einsum config (jasl convention) rather
        # than b12x's o_proj.compute_fp8_einsum_recipe(): on SM12x the latter
        # returns the SM100 recipe (1,1,128) (scale_out_blocks=1024), which the
        # vendored deepseek_v4_fp8_einsum rejects ("scale rows must be divisible
        # by 1024, got 32"). The vendored config returns (1,128,128) for SM12x.
...
class DeepSeekV4DSparkModel(nn.Module):
    ...
    def draft(
        ...
        *,
        ...
        # draft_input_ids[:,0] is the BONUS token (next_token_ids), which lives at
        # absolute position last_context+1 (one past the last verified context
        # token, whose position is main_positions[:,-1]). The previous code used
        # offsets [0, gamma) which RoPE-encoded the whole draft block one step too
        # early (bonus token at the last-context position instead of +1),
        # detuning every draft<->context dot-product and depressing position-0
        # acceptance below even MTP. Use [1, gamma+1] so the bonus token sits at
        # its true position last_context+1.
