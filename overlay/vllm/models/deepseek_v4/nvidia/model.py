...
class DeepseekV4Model(nn.Module):
    ...
    def forward(
        ...
        # EAGLE3 / DSpark: collect (HC-reduced) hidden states at the configured
        # aux layers to seed the DSpark draft.
        ...
        for idx, layer in enumerate(
            ...
            if (
                ...
                # and reduce over the hidden-chain dim (h.mean(dim=hc)), matching
                # the DSpark reference. Side copy so the loop's deferred mixes
                # are untouched. MUST mirror the main path's MHC branch: under
                # torch.compile/cudagraph the tilelang mhc_post is uncapturable
                # ("function marked as skipped" — tilelang JIT introspects its
                # own source); b12x_mhc_post is the compile/capture-safe kernel.
