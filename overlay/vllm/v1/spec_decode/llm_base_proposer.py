...
class SpecDecodeBaseProposer:
    ...
    def propose(
        ...
        if self.method in ("eagle3", "dflash", "dspark"):
            ...
            if isinstance(model, BreakableCUDAGraphWrapper):
                model = model.unwrap()
            # DSpark uses its own draft class (DeepSeekV4DSpark); skip the
            # eagle/dflash model-class assert for it.
            if self.method != "dspark":
                assert isinstance(
                    model,
                    (
    ...
    def load_model(self, target_model: nn.Module) -> None:
        ...
        # DSpark's draft class (DeepSeekV4DSpark) computes logits via its own
        # heads in the proposer hot path and has no compute_logits method.
